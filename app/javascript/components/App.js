...
export default function App() {
  ...
  const deleteItem = (listName, id) => {
    if (listName === "portfolio") {
      ...
      return axios
        .delete(`http://localhost:3000/api/portfolio_items/${id}`)
        .then(() => {
          setAppData((prev) => ({
            ...prev,
    ...
    if (listName === "watchlist") {
      const newWatchlistItems = removeOneFromList(
        listName,
        appData.watchlistItems,
        id
      );

      return axios
        .delete(`http://localhost:3000/api/watchlist_items/${id}`)
        .then(() => {
          setAppData((prev) => ({
            ...prev,
  ...
  const updatePortfolioItem = (newShares, id, asset_id) => {
    const newPortfolioItems = updateSharesForItem(
      appData.portfolioItems,
      newShares,
      id
    );
    ...
    return axios
      .patch(`http://localhost:3000/api/portfolio_items/${id}`, {
        portfolio_item,
      })
      .then(() => {
        setAppData((prev) => ({
    ...
    <BrowserRouter>
      <ThemeProvider theme={theme}>
        ...
        <Routes>
          <Route
            path="/"
            element={<Dashboard portfolioItems={appData.portfolioItems} />}
          />
          <Route
            path="/portfolio"
            element={
